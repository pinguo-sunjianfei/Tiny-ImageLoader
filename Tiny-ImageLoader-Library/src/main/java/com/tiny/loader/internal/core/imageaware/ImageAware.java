 ...
 *******************************************************************************/
package com.tiny.loader.internal.core.imageaware;
 ...
 * through {@link com.tiny.loader.internal.core.ImageLoader TinyImageLoader}.
 * It can wrap any Android {@link android.view.View View} which can be accessed by {@link #getWrappedView()}. Wrapped
 * view is returned in {@link com.tiny.loader.internal.core.listener.ImageLoadingListener ImageLoadingListener}'s
 * callbacks.
 *
...
public interface ImageAware {
	 ...
	 * Is called on UI thread if TinyImageLoader was called on UI thread. Otherwise - on background thread.
	 */
	int getWidth();
	...
	/**
	 * Returns {@linkplain com.tiny.loader.internal.core.assist.ViewScaleType scale type} which is used for
	...
	/**
	 * Returns a flag whether image aware view is collected by GC or whatsoever. If so then TinyImageLoader stop processing
	 * of task for this image aware view and fires
	 * {@link com.tiny.loader.internal.core.listener.ImageLoadingListener#onLoadingCancelled(String,
	 * android.view.View) ImageLoadingListener#onLoadingCancelled(String, View)} callback.<br />
	 * Mey be called on UI thread if TinyImageLoader was called on UI thread. Otherwise - on background thread.
	 *
	 * @return <b>true</b> - if view is collected by GC and TinyImageLoader should stop processing this image aware view;
	 * <b>false</b> - otherwise
	 */
	boolean isCollected();
	 ...
	 * image view instance and should be the same for same instances. This ID identifies processing task in TinyImageLoader
	 * so TinyImageLoader won't process two image aware views with the same ID in one time. When TinyImageLoader get new task
	 ...
	 * {@linkplain com.tiny.loader.internal.core.DisplayImageOptions.Builder#showImageForEmptyUri(
	 *android.graphics.drawable.Drawable) for empty Uri},
	 * {@linkplain com.tiny.loader.internal.core.DisplayImageOptions.Builder#showImageOnLoading(
	 *android.graphics.drawable.Drawable) on loading} or
	 * {@linkplain com.tiny.loader.internal.core.DisplayImageOptions.Builder#showImageOnFail(
	 *android.graphics.drawable.Drawable) on loading fail}. These drawables can be specified in
	 * {@linkplain com.tiny.loader.internal.core.DisplayImageOptions display options}.<br />
	 * Also can be called in {@link com.tiny.loader.internal.core.display.BitmapDisplayer BitmapDisplayer}.< br />
	 * Is called on UI thread if TinyImageLoader was called on UI thread. Otherwise - on background thread.
	 *
	 ...
	 * {@link com.tiny.loader.internal.core.display.BitmapDisplayer BitmapDisplayer}.< br />
	 * Is called on UI thread if TinyImageLoader was called on UI thread. Otherwise - on background thread.
	 *
