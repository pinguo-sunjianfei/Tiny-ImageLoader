 ...
 *******************************************************************************/
package com.tiny.loader.internal.utils;
...
public final class ImageSizeUtils {
     ...
     * {@link com.tiny.loader.internal.core.imageaware.ImageAware view} parameters, configuration
    ...
    public static int computeImageSampleSize(ImageSize srcSize, ImageSize targetSize, ViewScaleType viewScaleType,
        ...
        scale = considerMaxTargetSize(srcWidth, srcHeight, targetWidth, targetHeight, scale, powerOf2Scale);
        scale = considerMaxTextureSize(srcWidth, srcHeight, scale, powerOf2Scale);

        return scale;
    }

    private static int considerMaxTargetSize(int srcWidth, int srcHeight, int targetWidth, int targetHeight, int scale, boolean powerOf2) {
        while ((srcWidth / scale) > targetWidth || (srcHeight / scale) > targetHeight) {
            if (powerOf2) {
                scale *= 2;
            } else {
                scale++;
